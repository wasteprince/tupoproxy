...
образ `ghcr.io/wasteprince/tupoproxy-caddy-l4:v3.8.3` для `amd64` или `arm64`
вместе с релизом tupoproxy.

Контейнер использует host network и передаёт FakeTLS на
`127.0.0.1:18443`. Это исключает Docker bridge, `docker-proxy` и правила
межсетевого экрана между Caddy и tupoproxy. Управляемый Caddy не публикует и
не занимает дополнительные порты: HTTP-редиректы и admin API отключены.
Проверка после установки обращается к этому же `443` через loopback, поэтому
VPS без NAT hairpin больше не вызывает ложный откат рабочей конфигурации.

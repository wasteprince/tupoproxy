...
| Защита от проб | Неверная авторизация уходит на настоящий HTTP/2 decoy с отдельным сертификатом |
...
flowchart LR
    ...
    T -->|браузер или неверный ключ| D[Изолированный или внешний decoy HTTPS<br>отдельный сертификат]
...
на сервер, и отдельный FakeTLS decoy-домен. Decoy может указывать как на другой
HTTPS-сервер, так и на тот же VPS. Если оба домена имеют общий IP и на выбранном
decoy-порту ещё нет доверенного HTTP/2 HTTPS, установщик сам выпустит для decoy
отдельный сертификат и поднимет его в изолированной конфигурации. Rust, Cargo и
...
2. Отдельный FakeTLS decoy-домен; он может указывать на этот же VPS.
3. HTTPS-порт decoy — любой свободный порт, отличный от публичного порта прокси.
...
бинарник, поставит зависимости, получит необходимые сертификаты, создаст
secret, настроит cover-сайт, HAProxy и systemd, откроет порт в активном UFW,
запустит всё и выведет готовую Telegram-ссылку. Результат также сохранится в
...
decoy SNI — настоящий сертификат decoy-домена и `h2`. Установка
завершается успешно только тогда, когда оба маршрута действительно работают.
После запуска сервисов проверка ждёт их готовности до 60 секунд, поэтому
краткая инициализация локального decoy не приводит к ложной ошибке установки.
...
Сертификаты и DNS-credentials по умолчанию сохраняются, потому что они могут
использоваться другим сайтом. Удалить также сертификаты, выпущенные установщиком:
...
- выпускает или подключает отдельные сертификаты origin и локального decoy;
...
куда отправляются неавторизованные проверки. Для внешнего decoy на нём уже
должен работать доверенный HTTP/2 HTTPS. Если decoy указывает на тот же VPS и
HTTPS ещё не настроен, установщик создаёт локальный listener только внутри
изолированных сервисов tupoproxy. Публичный и decoy-порты должны различаться.
...
Ручной пример ниже использует внешний decoy: создайте DNS-записи `A`, а при
наличии IPv6 — `AAAA`, только для origin. Для автоматического same-server
варианта используйте однокомандный установщик выше — он отдельно выпускает
сертификат и не меняет глобальный nginx-конфиг.
...
`openssl` должен показать сертификат decoy и `ALPN protocol: h2`, а

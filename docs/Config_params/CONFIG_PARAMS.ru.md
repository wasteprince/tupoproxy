# Справочник параметров конфигурации tupoproxy
...
## default_dc
  ...
  - **Описание**: DC по умолчанию, используемый для нестандартных DC. Когда клиент запрашивает неизвестный/нестандартный DC без переопределения, tupoproxy направляет его в этот кластер по умолчанию.
  - **Пример**:
...
## data_path
  ...
  - **Описание**: Необязательный путь к каталогу данных состояния tupoproxy.
  - **Пример**:
...
## use_middle_proxy
  - **Ограничения / валидация**: `bool`.
  - **Описание**: Включает режим ME; если значение `false`, tupoproxy возвращается к прямой DC-маршрутизации.
  - **Пример**:
...
## proxy_secret_url
  ...
  - **Описание**: Необязательный URL для получения файла `proxy-secret` используемого ME-handshake/аутентификацией RPC. tupoproxy всегда сначала пытается выполнить новую загрузку с этого URL (если не задан, используется https://core.telegram.org/getProxySecret).
  - **Пример**:
...
## proxy_config_v4_cache_path
  ...
  - **Описание**: Необязательный путь к кэшу для необработанного (raw) снимка getProxyConfig (IPv4). При запуске tupoproxy сначала пытается получить свежий снимок; в случае сбоя выборки или пустого снимка он возвращается к этому файлу кэша, если он присутствует и не пуст.
  - **Пример**:
...
## proxy_config_v4_url
  ...
  - **Описание**: Необязательный URL для получения `getProxyConfig` (IPv4). tupoproxy при всегда пытается выполнить новую загрузку с этого URL (и если не задан, использует `https://core.telegram.org/getProxyConfig`).
  - **Example**:
...
## proxy_config_v6_cache_path
  - **Ограничения / валидация**: `String`. Если используется, значение не должно быть пустым или содержать только пробелы.
  - **Описание**: Необязательный путь к кэшу для необработанного (raw) снимка getProxyConfigV6 (IPv6). При запуске tupoproxy сначала пытается получить свежий снимок; в случае сбоя выборки или пустого снимка он возвращается к этому файлу кэша, если он присутствует и не пуст.
  - **Пример**:
...
- **Описание**: Необязательный URL для получения `getProxyConfigV6` (IPv6). tupoproxy при всегда пытается выполнить новую загрузку с этого URL (и если не задан, использует `https://core.telegram.org/getProxyConfigV6`).
- **Example**:
...
## upstream_connect_failfast_hard_errors
  - **Ограничения / валидация**: `bool`.
  - **Описание**: Если включено (true), tupoproxy пропускает дополнительные повторные попытки для постоянных ошибок подключения к upstream.
  - **Пример**:
...
## ipv6
  ...
  - **Пример**:
    ...
    ```
## prefer
  - **Ограничения / валидация**: Должно быть `4` или `6`. Если `prefer = 4`, а `ipv4 = false`, tupoproxy принудительно использует `prefer = 6`. Если `prefer = 6`, а `ipv6 = false`, tupoproxy принудительно использует `prefer = 4`.
...
## stun_use
  ...
  - **Пример**:
    ...
    ```
## stun_servers
  - **Ограничения / валидация**: `String[]`. Значения обрезаются; пустые значения удаляются; список очищается от дубликатов. Если этот ключ **не** задан явно, tupoproxy использует встроенный список STUN по умолчанию.
...
## listen_unix_sock_perm
  ...
  - **Пример**:
    ...
    ```
## listen_tcp
  - **Ограничения / валидация**: `bool` (необязательный параметр). Если этот параметр не задан, tupoproxy автоматически использует:
...
- `false`, если задан `listen_unix_sock`.
  ...
  - **Пример**:
    ...
    ```
## client_mss
  - **Ограничения / валидация**: `String`. Пустое значение или отсутствие параметра означает, что tupoproxy не изменяет MSS, выбранный ядром. Поддерживаемые presets: `"extreme-low"` = `88`, `"tspu"` = `92`, `"2in8"` = `256`. Пользовательское десятичное значение должно быть строкой в диапазоне `88..=4096`.
  - **Описание**: Управляет размером сегментов в клиентских соединениях. По умолчанию значение применяется к TCP listener и действует в течение всего соединения. Если в Linux также задан `client_mss_bulk`, tupoproxy временно ограничивает `TCP_MAXSEG` принятого socket значением `client_mss` на время отправки начального аутентифицированного FakeTLS-ответа (`ServerHello`), а затем восстанавливает bulk MSS при успехе, ошибке записи или отмене задачи. Настройка не влияет на API, metrics, Unix sockets, Telegram upstreams, ME sockets и mask backend connections. Изменение требует перезапуска или повторного создания listener.
  - **Operator note**: Two-tier `synlimit` profile больше не требует автоматического отключения MSS внутри tupoproxy. Оператор должен сам решить, оставлять MSS shaping для handshake fragmentation или отключать его ради более высокой скорости media.
...
## backend
  ...
  - **Описание**: Выбор набора инструментов для применения notrack-правил.
    ...
    - **`nftables / iptables`**: принудительно выбирает соответствующий backend; при отсутствии бинарника правила не применяются. В nft-режиме используется таблица `inet tupoproxy_conntrack`, в `iptables` — цепочка TUPOPROXY_NOTRACK в таблице raw.
  - **Пример**:
...
## reuse_allow
  - **Ограничения / валидация**: `bool`.
  - **Описание**: Включает `SO_REUSEPORT` для совместного использования привязки нескольких экземпляров (позволяет нескольким экземплярам tupoproxy прослушивать один и тот же `ip:port`).
  - **Пример**:
...
## tls_domains
  ...
  - **Пример**:
    ...
    ```
## tls_fingerprints
  - **Ограничения / валидация**: ключ — корректное DNS-имя SNI; значение — `"chrome"`, `"firefox"`, `"compat"` или `"legacy"`. Домены автоматически добавляются в набор создаваемых `ee`-ссылок. Изменение требует перезапуска.
  - **Описание**: связывает домен, записанный в стандартном `ee` credential, с профилем исходящего TLS-пробника и формой TLS-record'ов ответа. Это серверная настройка: она не может изменить JA3/JA4 входящего ClientHello, который формирует приложение Telegram.
  - **Пример**:
...
## mask_host
  - **Ограничения / валидация**: `String` (необязательный параметр).
    ...
    - Если не задан параметр `mask_host` и `mask_unix_sock` не задан, tupoproxy по умолчанию устанавливает для `mask_host` значение `tls_domain`.
...
## exclusive_mask
  ...
  - **Описание**: Per-SNI TCP targets для fallback-трафика. Если SNI в TLS ClientHello совпадает с ключом, tupoproxy проксирует это неаутентифицированное соединение на указанный target. Остальной fallback-трафик продолжает использовать существующий `mask_host`/`mask_port` или SNI-aware default masking behavior.
  - **Пример**:
...
## mask_unix_sock
  ...
  - **Пример**:
    ...
    ```
## fake_cert_len
  - **Ограничения / валидация**: `usize`. Когда `tls_emulation = false` и используется значение по умолчанию, tupoproxy может рандомизировать его при запуске для обеспечения вариативности.
...
Когда `mask_shape_hardening = true`, tupoproxy дополняет (padding) хвост потока **client → mask** до границы группы данных при завершении работы промежуточного звена:
...
- `mask_shape_bucket_cap_bytes`
Устанавливает наибольший bucket, до которого tupoproxy может выполнять padding по bucket-логике.
...
В агрессивном режиме tupoproxy все равно может применять shaping к такому backend-silent соединению, если от backend не было получено ответа.
...
## ipv4 (upstreams)
  ...
  - **Описание**: Разрешает IPv4 DC-targets для этого upstream. Если не задан, tupoproxy определяет поддержку автоматически по runtime-состоянию connectivity.
  - **Пример**:
...
## ipv6 (upstreams)
  - **Ограничения / валидация**: `bool` (необязательный параметр).
  - **Описание**: Разрешает IPv6 DC-targets для этого upstream. Если не задан, tupoproxy определяет поддержку автоматически по runtime-состоянию connectivity. Установите `true`, если upstream proxy доступен с локального хоста по IPv4, но сам proxy умеет подключаться к Telegram DC по IPv6.
  - **Пример**:
...
## bind_addresses
  - **Ограничения / валидация**: `String[]` (необязательный параметр). Применяется в случае, если `type = "direct"`.
    ...
    - Во время выполнения tupoproxy выбирает адрес, соответствующий целевому семейству (IPv4 или IPv6). Если установлен параметр «bind_addresses», и ни один из них не соответствует целевому семейству, попытка подключения считается неудачной.
...
## user_id
  ...
  - **Описание**: User ID для команды CONNECT в SOCKS4. Примечание: если для запроса выбран scope, tupoproxy может переопределить это значение на выбранный scope.
  - **Пример**:
...
## username
  ...
  - **Описание**: Имя пользователя SOCKS5 (для аутентификации по username/password). Примечание: если для запроса выбран scope, tupoproxy может переопределить это значение на выбранный scope.
  - **Пример**:
...
## password
  - **Ограничения / валидация**: `String` (необязательный параметр). Используется только при `type = "socks5"`.
  - **Описание**: Пароль SOCKS5 (для аутентификации по username/password). Примечание: если для запроса выбран scope, tupoproxy может переопределить это значение на выбранный scope.
  - **Пример**:
